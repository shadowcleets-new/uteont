...
export function LiveSiteScraper() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const data = (await res.json().catch(() => null)) as
        | { runId?: number; score?: number; crawled?: number; issueCount?: number; error?: string; message?: string }
        | null;
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <form onSubmit={handleSubmit} className="rounded-[10px] border border-[#e8e6dc] bg-white p-5">
        ...
        <div className="flex flex-col md:flex-row gap-2 mb-3">
          ...
          >
            ...
            {submitting ? "Crawling…" : "Run Site Scan"}
          </button>
        </div>
        <input
          type="text"
        ...
        />
        {error && <div className="mt-3 text-[12px] text-[#a33b2b] font-mono">{error}</div>}
        <p className="mt-3 text-[11px] text-[#9a988e] font-serif italic">
          Runs the Site Crawl agent against the competitor right now — public
          HTML only, no credentials needed. Results land in the Directory tab
          and the runs history.
        </p>
      </form>
      ...
      <div className="rounded-[10px] border border-[#e8e6dc] bg-white">
        <div className="px-5 py-3 border-b border-[#e8e6dc] flex items-center justify-between">
          <div className="text-[10px] font-bold tracking-wider text-[#9a988e]">RECENT SCANS</div>
          <div className="text-[11px] text-[#6b6a64] tabular-nums">{recent.length}</div>
        </div>
        {recent.length === 0 ? (
          <div className="px-5 py-6 text-[12px] text-[#9a988e] italic font-serif">
            No scans this session — run one above.
          </div>
        ) : (
            ...
            {recent.map((r) => (
              <li key={`${r.runId}-${r.recordedAt}`} className="px-5 py-3">
                <div className="flex items-center justify-between gap-3 mb-0.5">
                  <span className="font-mono text-[#141413] truncate">{r.url}</span>
                  <span className="shrink-0 flex items-center gap-2">
                    {r.score != null && (
                      <span
                        className="text-[10px] font-bold tabular-nums"
                        style={{ color: r.score >= 70 ? "#788c5d" : r.score >= 40 ? "#d97757" : "#a33b2b" }}
                      >
                        CRAWL {r.score}
                      </span>
                    )}
                    {r.runId && (
                      <span className="text-[10px] text-[#9a988e] tabular-nums">run #{r.runId}</span>
                    )}
                  </span>
                </div>
                <div className="text-[11px] text-[#6b6a64]">
                  {new Date(r.recordedAt).toLocaleString()}
                  {r.crawled != null && <> · {r.crawled} pages</>}
                  {r.issueCount != null && <> · {r.issueCount} issues</>}
                  {r.note && <> · {r.note}</>}
                </div>
              </li>
            ))}
